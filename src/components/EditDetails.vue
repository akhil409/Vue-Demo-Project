<template>
       <v-container class="form" >
                       ...
                       <v-form ref="signUpForm" class="formNav" v-for="(value,index) in previousDetails" :key="index" v-model="formValidity" @submit.prevent="updateDetails">
                            ...
                            <hr>
                            <!-- Title -->
                            <v-text-field label="Title" v-model.trim="value.title"  @blur="$v.title.$touch()" required placeholder="Enter your title" >{{value.title}}</v-text-field>
                            ...
                            <!-- Firstname -->
                            <v-text-field label="Firstname" v-model.trim="value.firstName"  @blur="$v.firstName.$touch()" required placeholder="Enter your firstname" ></v-text-field>
                            ...
                            <!-- Lastname -->
                            <v-text-field label="Lastname" v-model.trim="value.lastName"  @blur="$v.lastName.$touch()" required placeholder="Enter your lastname" ></v-text-field>
                            ...
                            <!-- Email -->
                            <v-text-field label="Email" type="email" v-model.trim="value.email" @blur="$v.email.$touch()" required placeholder="Enter your Email" ></v-text-field>
                           ...
                           <!-- age -->
                            ...
                            <router-link to="/post-details">
                                    <v-btn class="mr-4" type="submit" color="primary" disabled="!formValidity"  >Update</v-btn>
                            </router-link>
...
export default {
    ...
    getPreviousData(){
            axios.get(`https://passport-registration-app-default-rtdb.firebaseio.com/posts.json/+${this.$route.params.id}`).then((response) => {
        ...
        formatePostDetails(posts) {
            for (let key in posts) {
                this.previousDetails.push({ ...posts[key], id: key });
            }
            console.log(this.previousDetails);
        },
        updateDetails(){
                alert('Details Updated Successfully')
                console.log(`The updated deatails are ${this.title} ${this.firstName} ${this.lastName} ${this.email} ${this.mobileNumber} ${this.organization} ${this.designation} ${this.address} ${this.password} ${this.confirmPassword} ${this.dob} ${this.age} ${this.browers} ${this.gender} ${this.images} `)
                
                axios.put(`https://passport-registration-app-default-rtdb.firebaseio.com/posts.json/${this.$route.params.id}`,{ 
                                                                                           Title:this.title,
            ...
            .then((res)=>{
                     this.isSuccess = true;
                     console.log('Details Updated in Firbase successfully',res);
                     this.$emit('postCreated');
                     this.$router.push(`/post-details`)
            }).catch((err)=>{
                     console.log('Error while Updating Data in Firebase',err);
            })
        },
        //  resetValidation(){
